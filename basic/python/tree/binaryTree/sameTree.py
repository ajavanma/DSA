...
class Solution:
    def isSameTree(self, p: Optional[TreeNode], q: Optional[TreeNode]) -> bool:
        ...
        if not p and not q:
            return True
        # If one of the nodes is None or the values do not match, trees are not the same
...
sol = Solution()
print(sol.isSameTree(node1, node2))  # Output: True

node3 = TreeNode(1, TreeNode(2))
node4 = TreeNode(1, None, TreeNode(2))
print(sol.isSameTree(node3, node4))  # Output: False
